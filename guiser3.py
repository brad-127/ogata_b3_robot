...
arm1=90 #0-180 このコードの中での角度
arm2=90 #0-180
arm3=90 #0-180
arm4=90 #20-90

arm1_=90 #0-180 実際の角度
arm2_=90 #0-180
arm3_=90 #0-180
arm4_=90 #20-90
...
def send_value():
    ...
    while(arm1 > arm1_) :
        arm1_+=1
        inkeys = bytes('a', 'utf-8')
        #print(inkeys)
        ser.write(inkeys)
    while(arm1 < arm1_) :
        arm1_-=1
        inkeys = bytes('b', 'utf-8')
        #print(inkeys)
        ser.write(inkeys)
    while(arm2 > arm2_) :
        arm2_+=1
        inkeys = bytes('c', 'utf-8')
        #print(inkeys)
        ser.write(inkeys)
    while(arm2 < arm2_) :
        arm2_-=1
        inkeys = bytes('d', 'utf-8')
        #print(inkeys)
        ser.write(inkeys)
    while(arm3 > arm3_) :
        arm3_+=1
        inkeys = bytes('e', 'utf-8')
        #print(inkeys)
        ser.write(inkeys)
    while(arm3 < arm3_) :
        arm3_-=1
        inkeys = bytes('f', 'utf-8')
        #print(inkeys)
        ser.write(inkeys)
    while(arm4 > arm4_) :
        arm4_+=1
        inkeys = bytes('g', 'utf-8')
        #print(inkeys)
        ser.write(inkeys)
    while(arm4 < arm4_) :
        arm4_-=1
        inkeys = bytes('h', 'utf-8')
        #print(inkeys)
        ser.write(inkeys)
